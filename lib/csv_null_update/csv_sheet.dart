...
class CsvSheet {
    ...
    if(hasHeaderRow) {
      ...
      for(var i = 0; i < _rows.length; i ++) {
        ...
        if(_rows.indexOf(val, i+1) != -1) {
          throw 
             FormatException('The header column $val appears more than once');
        }
        _headers[val] = i;
      }
      _row = CsvRow(_headers);
  ...
  operator [](index) {
    if(index is String) {
      ...
      if(!_headers.containsKey(index)) throw RangeError('$tmp is not a valid column header');
